...
| One message, every surface | `DevPipeline._emit()` publishes one executive-composed string per milestone via `STREAMING_TOKEN` — the same text that becomes the chat message; there is no second narration/TTS surface to keep in sync with it (that surface was deliberately removed — see the narration-removal work referenced below). |
