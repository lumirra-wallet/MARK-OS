...
### Presence Engine (3D brain visualization)
- A procedurally-shaped, anatomically-landmarked brain (longitudinal fissure, temporal lobes, cerebellum/brainstem) — not a generic sphere — rendered with a dense static point-cloud surface texture, a subtle lit hull, and a Fresnel glow shell. (`artifacts/mark-dashboard/src/components/PresenceEngine.tsx`)
- Every visual signal (color, energy, pulse, particle bursts) is driven by real data: `agent.mind`'s actual mode/confidence/health, real WebSocket timeline events, real mic amplitude, real speech-playback state. No `setInterval`/`setTimeout` anywhere in the file — verified by direct inspection, not just claimed.
- Went through multiple real revisions this session based on direct visual comparison against a reference image the owner provided — not a one-shot guess.
